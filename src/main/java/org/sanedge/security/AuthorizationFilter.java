...
public class AuthorizationFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext containerRequestContext) throws IOException {
        ...
        if (!isSecurityOptional(resourceMethod, resourceClass)) {
            ...
            List<ERole> classRoles = extractRoles(resourceClass);
            List<ERole> methodRoles = extractRoles(resourceMethod);
    ...
    private void checkPermissions(List<ERole> allowedRoles, SecurityContext securityContext) {
        if (!isAccessAllowed(allowedRoles, securityContext)) {
            throw new ForbiddenException();
        }
    }

    private boolean isAccessAllowed(List<ERole> allowedRoles, SecurityContext securityContext) {
        for (ERole allowedRole : allowedRoles) {
    ...
    private List<ERole> extractRoles(AnnotatedElement annotatedElement) {
